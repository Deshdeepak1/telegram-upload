...
def get_file_display_name(message):
    ...
    if message.text:
        display_name_parts.append(
            f"[{message.text}]" if display_name_parts else message.text
        )
...
async def interactive_select_files(client, entity: str):
    ...
    iterator = amap(
        lambda x: (x, get_file_display_name(x)),
        iterator,
    )
...
async def interactive_select_local_files():
    iterator = filter(
        lambda x: os.path.isfile(x) and os.path.lexists(x), os.listdir(".")
    )
    ...
    def handle_parse_result(self, ctx, opts, args):
        if self.mutually_exclusive.intersection(opts) and self.name in opts:
            raise click.UsageError(
                ...
                "arguments `{}`.".format(self.name, self.mutually_exclusive_text)
            )
...
@click.option(
    "--to",
    default=None,
    help='Phone number, username, invite link or "me" (saved messages). '
    'By default "me".',
)
@click.option(
    "--config",
    default=None,
    help='Configuration file to use. By default "{}".'.format(CONFIG_FILE),
)
@click.option(
    "-d",
    "--delete-on-success",
...
@click.option(
    "--force-file",
    is_flag=True,
    help="Force send as a file. The filename will be preserved "
...
@click.option(
    ...
    "--forward",
    multiple=True,
    help="Forward the file to a chat (alias or id) or user (username, "
    "mobile or id). This option can be used multiple times.",
)
@click.option(
    "--directories",
    default="fail",
    type=click.Choice(list(DIRECTORY_MODES.keys())),
...
@click.option(
    "--large-files",
    default="fail",
    type=click.Choice(list(LARGE_FILE_MODES.keys())),
...
@click.option(
    "--caption",
    type=str,
    help="Change file description. By default the short name. "
...
@click.option(
    ...
    mutually_exclusive=["thumbnail_file"],
...
@click.option(
    ...
    default=None,
    cls=MutuallyExclusiveOption,
    mutually_exclusive=["no_thumbnail"],
...
def upload(
    ...
    files = files_cls(
        files, caption=caption, thumbnail=thumbnail, force_file=force_file
    )
    if large_files == "fail":
        # Validate now
        files = list(files)
    if directories == "recursive" and caption == "{path}":
        directory_id = int(time.time())
        client.send_message(to, f"{directory_id}_start")
    if album:
    ...
    if directories == "recursive" and caption == "{path}":
        client.send_message(to, f"{directory_id}_end")
...
@click.option(
    ...
    "directory_id",
    default="",
    help="Directory id created while uploading. Use to recreate directory structure.",
)
def download(from_, config, delete_on_success, proxy, interactive, directory_id):
    ...
    if interactive:
        ...
        messages = client.find_directory(from_, directory_id)
    else:
        messages = client.find_files(from_)
    client.download_files(from_, messages, delete_on_success, bool(directory_id))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        sys.stderr.write(
            "A command is required. Available commands: {}\n".format(
                ", ".join(commands)
            )
        )
        sys.exit(1)
    if sys.argv[1] not in commands:
        sys.stderr.write(
            "{} is an invalid command. Valid commands: {}\n".format(
