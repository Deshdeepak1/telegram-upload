...
def get_message_file_attribute(message):
    return next(
        filter(
            lambda x: isinstance(x, DocumentAttributeFilename),
...
def parse_proxy_string(proxy: Union[str, None]):
    ...
    if proxy_parsed.scheme == "mtproxy":
        return (
            ...
            proxy_parsed.port,
            proxy_parsed.username,
        )
    try:
        import socks
    except ImportError:
        raise TelegramProxyError(
            "pysocks module is required for use HTTP/socks proxies. "
...
class Client(TelegramClient):
    ...
    def send_files_as_album(
        self, entity, files, delete_on_success=False, print_file_id=False, forward=()
    ):
    ...
    def _send_file_message(self, entity, file, thumb, progress):
        message = self.send_file(
            entity,
            file,
            thumb=thumb,
            file_size=file.file_size if isinstance(file, File) else None,
            caption=file.file_caption,
            force_document=file.force_file,
            progress_callback=progress,
            attributes=file.file_attributes,
        )
        if (
            hasattr(message.media, "document")
            and file.file_size != message.media.document.size
        ):
    ...
    def send_files(
        ...
        for file in files:
            ...
            if print_file_id:
                click.echo(
                    'Uploaded successfully "{}" (file_id {})'.format(
                        file.file_name, pack_bot_file_id(message.media)
                    )
                )
    ...
    def find_directory(self, entity, directory_id):
        found = False
        for message in self.iter_messages(entity):
            if not found:
                if message.message == f"{directory_id}_end":
                    found = True
                else:
                    continue
            if message.document:
                ...
                if message.message == f"{directory_id}_start":
                    break
                else:
                    continue
        ...
        for message in messages:
            ...
            try:
                if create_directories:
                    self.download_media(
                        message, message.message, progress_callback=progress
                    )
                else:
